...
public class UI {
	...
	int dialogueIndex = 0;
	int playerFace = 0;
	public int entityAttackPlayer = 0;
			...
			if (gp.gameState == gp.battleState) {
				...
				if (monsterName == 0) {
					...
					try {
						...
						g2.drawImage(image, 250 - gp.getOrigianlCharacterSize(), 150 - gp.getOrigianlCharacterSize(),
								gp.getOrigianlCharacterSize() * 8, gp.getOrigianlCharacterSize() * 5, null);
				...
				if (gp.ui.scriptNum == 0) {
					drawBattleOptions(g2);
				} else {
					drawDialogue(g2);
				}
			}
	...
	private void drawBattleOptions(Graphics2D g2) {
		String text = "ATTACK";
		int x = getXforCentredText(text, g2);
		int y = 8 * gp.getOrigianlCharacterSize();
		g2.drawString(text, x, y);
	...
	private void drawDialogue(Graphics2D g2) {
		...
		if (gp.ui.scriptNum == 1) {
			gp.battleManager.adjustLife(0);
		} else if (gp.ui.scriptNum == 2) {
			gp.battleManager.adjustLife(1);
		}
	}
	...
	public void drawStoryEnd(Graphics2D g2) {
		g2.setFont(scriptFont);
		g2.setColor(Color.black);
		...
		g2.setColor(Color.white);
		if (scriptNum == 0) {
			drawCenteredString(g2, "당신은 ", 5);
			drawCenteredString(g2, "모든 몬스터를 물리치는데 성공했다", 6);
			drawCenteredString(g2, "▶ SPACE", 9);

		} else if (scriptNum == 1) {
			drawCenteredString(g2, "당신은 마을로 돌아갔습니다", 4);
			drawCenteredString(g2, "마을 사람들은", 6);
			drawCenteredString(g2, "그 소식을 듣고 환호합니다", 7);
			drawCenteredString(g2, "▶ SPACE", 9);

		} else if (scriptNum == 2) {
			drawCenteredString(g2, "당신은 마을을 구하는데 성공했습니다", 5);
			drawCenteredString(g2, "축하드립니다", 6);
			drawCenteredString(g2, "▶ SPACE", 9);
		} else if (scriptNum == 3) {
			drawCenteredString(g2, "게임을 종료합니다.", 4);
			drawCenteredString(g2, "YES", 6, commandNum == 0);
			drawCenteredString(g2, "NO", 7, commandNum == 1);
		}
	}

	private void drawCenteredString(Graphics2D g2, String text, int yMultiplier) {
		drawCenteredString(g2, text, yMultiplier, false);
	}

	private void drawCenteredString(Graphics2D g2, String text, int yMultiplier, boolean highlight) {
		int x = getXforCentredText(text, g2);
		int y = yMultiplier * gp.getOrigianlCharacterSize();
		g2.drawString(text, x, y);
		if (highlight) {
			g2.drawString("▶", x - gp.getOrigianlCharacterSize(), y);
		}
	}
