...
class Story {
	...
	void pup() {
		...
		do {
			...
			if (input == 1) {
				// 정보 묻는다면 forest 값 해금해서 숲 들어갈 수 있도록 처리
				Script.Pubforest();
		...
		do {
			...
			if (input == 1) {
				...
				for (String itemName : itemNames) {
					item_list item = main_character.HP_item.get(itemName);
					System.out.println(itemName + " hp " + item.hp + "회복");
				}

				input = printScript(); // 사용자가 아이템을 선택
				if (input >= 1 && input <= itemNames.length) {
					// 선택한 아이템으로 회복
					String selectedItem = itemNames[input - 1];
					playerHP += main_character.HP_item.get(selectedItem).hp;
					System.out.println("\n당신은 " + main_character.HP_item.get(selectedItem).hp + "체력을 회복했다");
				}
			}
			...
			if (input == 1) {
				...
				if (input == 1) {
					...
					do {
						...
						if (input == 1) {
							/*
							 * 성배 탐색 현재 성수를 가진 상태인지 검사(신전 방문 이후 성수 드랍) 만약 성배 탐색이 처음이라면 temper 1로 값 변경 통해 성배
							 * 탐색했음을 알림
							 */
	...
	void forestmonster(int n, int place) {
		...
		if (ran != 2) {
			...
			if (playerHP <= 0) {
				...
				main_character.levelup(main_character.getMP());
			}
		}
	}

	static void story_end() {
		// 어떤 사유로든 게임을 종료할 시 실행함; 프로그램 종료
		System.out.println("\n게임을 종료합니다.\n감사합니다.");
		System.exit(0);

	}

	// 몬스터 사냥 이후 게임 결과를 출력하는 클래스
	static class GameResult {
		static void defeat() {
			System.out.println("플레이어가 사망하였습니다..."); // 만약 게이머가 죽었다면 안내 후 게임 종료
			Story.story_end();
		}

		static void defeatMonster(String monsterName) { // 만약 몬스터가 죽었다면 mp 증가 후 게임 종료
			System.out.println("몬스터를 처치하였습니다!");
			System.out.println("당신은 " + monsterName + "를 처치했습니다.\n");

			double ran = Math.random();

			// 신전을 다녀온 이후라면 성수를 얻을 수 있도록 함
			// 검사 요소 : 신전을 다녀왔는가 / 랜덤한 수를 뽑았는데 1/2 확률로 0.5 초과 값인가?
			if (main_character.temper == 1 && ran > 0.5) {
	...
	class END {
		public static void ENDING() {
			...
			// 게이머 정보 출력; 이름, 직업, 레벨, 가진 돈, 처리 몬스터 수
	...
	public class Game {
		...
		public static void main(String[] args) {
			...
			story = new Story();
			char ch = 'a';
			Script.MainScreen(); // 게임을 시작하는 문구를 출력함.
			start = sc.nextInt();
